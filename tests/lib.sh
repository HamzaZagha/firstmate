...
# iff the test's fakebin provides it. git stays in the farm on purpose: the
# suites' own fixture repos need the real binary, so its MISSING line is not
# assertable this way. Built once per test process and reused. It sets a
# global rather than echoing, and must be called directly (never in a $(...)
# substitution): fm_test_tmproot's cleanup trap fires when a substitution
# subshell exits, which would delete the farm it just built. The farm is
# populated with plain `ln -s <dir>/* <farm>/` (portable; GNU-only `cp -sn`
# is deliberately avoided) - an existing name never gets overwritten, so the
# first dir listed wins, matching PATH resolution order.
