...
Claude 2.x also pads its unbordered `❯` prompt row with U+00A0 no-break spaces, which the shared classifier normalizes before classifying; before that fix every claude submit verification false-errored "Enter swallowed" even when the text landed (see `docs/tmux-backend.md` "Incident (2026-07-09)").
