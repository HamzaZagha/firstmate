...
FM_STALE_ESCALATE_SECS=240         # idle seconds between wedge re-checks of an absorbed provably-working stale pane; a re-check that finds the crew stopped escalates, and stale panes whose crew is not provably working (and not idling at an already-surfaced finished status) surface immediately unless they declare the pause verb
