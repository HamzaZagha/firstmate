...
FM_FINISHED_RE='done:|PR ready|checks green|ready in branch|merged'   # finished-shaped subset eligible for the watcher's already-surfaced stale dedup
FM_STALE_ESCALATE_SECS=240         # idle seconds between wedge re-checks of an absorbed provably-working stale pane; a re-check that finds the crew stopped escalates, and stale panes whose crew is not provably working (and not idling at an already-surfaced finished status) surface immediately
FM_WEDGE_DEMAND_INSPECT_COUNT=3    # consecutive wedge-threshold escalations on the same unchanged pane before demand-deep-inspection is added
