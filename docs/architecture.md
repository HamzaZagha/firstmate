...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is not provably working, check-script output such as PR merge polling or an X-mode mention, stale panes whose crew is not provably working and whose last status is not an already-surfaced finished status, absorbed stale panes whose `FM_STALE_ESCALATE_SECS` re-check finds the crew no longer provably working, declared external waits that remain paused past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
A stale pane idling at a finished-shaped status (`FM_FINISHED_RE`, default `done:`/`PR ready`/`checks green`/`ready in branch`/`merged`) that already woke the first mate (its `.hb-surfaced-*` marker matches) is absorbed instead of re-tripping on every pane redraw, and a wedge-timer expiry re-checks the crew state: a re-check that still shows an actively-running run or busy pane keeps absorbing and resets the timer, so a long validation never wedge-escalates while its run is running.
That dedup never covers resumable verbs (`needs-decision:`, `blocked:`), which describe a crew expected to resume, so a crew that hard-stops at one keeps surfacing; a text steer through `fm-send.sh` clears the surfaced marker, so a steered crew that later hard-stops surfaces again.
Repeated wedge-threshold escalations on the same unchanged pane add an escalation count to the wake reason and, at `FM_WEDGE_DEMAND_INSPECT_COUNT`, a `demand-deep-inspection` marker.
