...
`[![:space:]]` trimming never strips U+00A0, so the leftover `❯` + NBSP matched neither the empty check nor the bare-prompt-glyph case and classified `pending` - on the idle composer, on the popup-open typed row, and on the running-turn row alike.
...
Fix: the shared composer classifier (`fm_composer_classify_content`, `bin/fm-composer-lib.sh`) normalizes U+00A0 to a plain space and re-trims before classifying, so every adapter that delegates to it - the tmux reader, herdr, orca, and cmux - gets the same normalization from the one owner.
...
Regression coverage: `tests/fm-composer-lib.test.sh` (the owner's unit cases), `tests/fm-send-claude-slash.test.sh` (tmux end-to-end), and the claude NBSP rows in `tests/fm-backend-herdr.test.sh`.
